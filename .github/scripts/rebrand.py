...
def main():
    print("Starting LYRA Rebrand Script...")
        ...
        for file in files:
            if file.endswith('.xml'):
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        content = f.read()
                    new_content = content.replace('>EchoMusic<', '>LYRA<').replace('>Echo Music<', '>LYRA<').replace('>EchoMusic Dev<', '>LYRA Dev<').replace('In EchoMusic,', 'In LYRA,').replace('"EchoMusic"', '"LYRA"')
    ...
    replace_in_file(welcome_dialog, r'Button\(\s*onClick\s*=\s*\{\s*uriHandler\.openUri\("https://github\.com/EchoMusicApp/Echo-Music"\)\s*\}.*?(?=Button\(\s*onClick\s*=\s*onDismissRequest)', '', flags=re.DOTALL)
    replace_in_file(welcome_dialog, r'text\s*=\s*"Echo Music"', 'text = "LYRA"')
    support_dialog = 'composeApp/src/commonMain/kotlin/echo/music/iad1tya/ui/component/SupportProjectDialog.kt'
    replace_in_file(support_dialog, r'Text\(\s*text\s*=\s*"If you enjoy EchoMusic.*?(?=}\s*},\s*confirmButton\s*=)', 'Text(text = "If you enjoy LYRA, thank you!", style = MaterialTheme.typography.bodyMedium, modifier = Modifier.padding(bottom = 8.dp))\n            ', flags=re.DOTALL)
