...
import echo.music.iad1tya.extension.getSizeOfFile
import echo.music.iad1tya.extension.zipInputStream
import echo.music.iad1tya.extension.zipOutputStream
...
actual suspend fun calculateDataFraction(cacheRepository: CacheRepository): SettingsStorageSectionFraction? {
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (mStorageStatsManager != null) {
            ...
            val echoMusicSize = getSizeOfFile(application.filesDir).bytesToMB()
            val thumbSize = (application.imageLoader.diskCache?.size ?: 0L).bytesToMB()
            val otherApp = echoMusicSize.let { usedSpace.minus(it) - thumbSize }
            val databaseSize =
                echoMusicSize - playerCache.bytesToMB() - downloadCache.bytesToMB() - canvasCache.bytesToMB()
            if (totalByte ==
                freeSpace + otherApp + echoMusicSize + thumbSize
            ) {
